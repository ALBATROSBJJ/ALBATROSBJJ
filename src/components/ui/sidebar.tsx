...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex h-16 flex-col justify-center p-2", className)}
      {...props}
    />
  )
})
...
>(({ className, showIcon = false, ...props }, ref) => {
  const [width, setWidth] = React.useState("75%"); // Start with a fixed default width

  React.useEffect(() => {
    // On the client, after mount, set a random width.
    // This avoids server-client mismatch.
